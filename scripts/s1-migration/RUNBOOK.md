...
→ employee-ids → elections → benefit-history → payments → hours → ledger
→ log-notes → cardchecks → enrollment-packet-tags → parity gates
...
- **hours before ledger** (permanent order, Task 414) — t20 maintains the
  id_map `payperiod` crosswalk (payperiod nid → `worker_hours.id`); t18
  resolves AR rows whose S1 `ledger_reference` targets a `sirius_payperiod`
  through it as `referenceType='hour'`. Ledger-before-hours leaves those rows
  degraded (`s1-unknown`) until the next t18 run's ref-heal pass. Data
  imported before this order existed is fixed ONCE via
  `repair-hour-links.ts` (§10.1) — never add the repair to a fleet.
...
- log-notes needs contacts AND workers (handler refs resolve to a real worker
  ...
  preserved as aggregate orphan counts and do not create placeholder workers
  or block the run. Workbook exclusions are explicit: bulk, audit/system and
  election-wizard events, system email, draft/sent letters, Twilio/STOP noise,
  raw SMF, and the retired communications disposition. The permitted member
  email, appeal-denial letter, and `material/material` document rows remain in
  notes. enrollment-packet-tags needs
...
long-running loader (contacts-workers, relationships, elections,
 benefit-history, payments, ledger, hours, log-notes, enrollment-packet-tags)
...
| 6 | `npx tsx scripts/s1-migration/load-relationships.ts` | `--allow-rejects future_start_date` | `future_start_date` is allowed by the 2026-08-26 fund ruling (mmcdermott4); it remains visible in the report and is not written because S2 storage rejects future starts. No-start rows load via the N26 default-dates ruling; prod expects `datesDefaulted ≈ 115`. Reconciliation may retarget worker endpoints in place when an alt contact gains its real worker; the migration-only storage path preserves the relation UUID while the normal staff/API update still forbids endpoint edits. Regression smoke: `scripts/oneoffs/s1-relationship-endpoint-retarget-smoke.ts`. | 24 relations, datesDefaulted=2 |
...
| 11 | `npx tsx scripts/s1-migration/load-hours.ts --migration-mode` | same | **Before ledger (12) — permanent order (Task 414).** After verify, the loader upserts the id_map `payperiod` crosswalk (every contributing payperiod nid → its monthly `worker_hours.id`, OVERWRITE semantics so retargeted periods repoint) and, on fully-verified runs, retires mappings not restamped this run (`detail.payperiodCrosswalk`: mapped/retired). **`--migration-mode` is REQUIRED on prod** (suppresses charge plugins — replay = double-billing; the loader preflight aborts if runnable charge plugins exist without it). Hard gate: `verifyMismatchCount: 0`. **`unresolvedWorker`/`unresolvedEmployer` are NOT expected to be 0 on a real extract** (first full real-target run 2026-08-09: 3363 / 7 month-groups) — the report now carries `unresolved*DistinctNids` + ≤20 sample nids per side; triage via 07 §P7: `staged_but_unmapped > 0` = loader gap (MUST fix before cutover), `not_staged` = deleted in S1 (documented skip, confirm with the §P7e/P7f LEFT JOIN node queries). `missing_json` will exceed the ~1,853 sparse-field baseline on any no-freeze extract (observed 3,462) — surplus rows are payperiods whose JSON row landed after the extract read the node (live hours imports racing the extract); they recur in every live extract and their pickup belongs to the parallel-run full-sync design, not to T20 (profile with §P7d/P7g). `legacy_json_format` skips are known-format legacy rows. `multiStatusMonths` counts SAME-employer mixed hour types within one month only — a worker with different statuses at different employers is the supported norm (status is per-employer) and never hits this counter. **No `--allow-rejects` gate by design:** hours has no reject gate — problem rows are counted skips, never fatal; the skip-block + §P7 triage IS the gate. `--stub-missing` is FORBIDDEN on a real target (dev-only crutch). **Sync (§10):** monthly rows converge via the `s1_staging.hours_keys` sidecar — every written (worker, employer, month) key is stamped after its flush; keys NOT restamped by the current run mark stale day=1 rows (payperiods deleted or month-retargeted in S1) for deletion + key removal (`detail.staleHoursCleanup`; skipped on `--dry-run` or when the run's own verify failed). First sync run over a pre-sync target: add `--adopt-hours-keys` once to seed the sidecar from existing mapped-pair rows. **Bulk write path (task 356):** in `--migration-mode` month-groups flush through `bulkUpsertWorkerHoursMigration` — bounded 500-row multi-row upserts that keep existing row ids and staff-owned `home`/`job_title`, write only migration-owned `hours` + `employment_status_id`, and skip per-row audit/event round trips (aggregate evidence only); per-flush deduplicated bulk worker-scan invalidation + denorm stale marking replace the per-row listeners (`detail.downstream`). **Interrupted runs resume with the SAME command — never reset or wipe**; stale cleanup stays unreachable until a run completes with verify pass. `detail.phaseStats` splits staged-row scan vs month-group write vs verify with phase-local rates. | 300 staged → 298 written+verified, 2 legacy skips |
| 12 | `npx tsx scripts/s1-migration/load-ledger.ts` | `--allow-rejects non_cleared_status` (2 Pending) | **After payments (10) AND hours (11) — permanent order (Task 414).** AR rows whose S1 `ledger_reference` targets a `sirius_payperiod` resolve through t20's id_map `payperiod` crosswalk to `referenceType='hour'` → `worker_hours.id` (reported in `detail.referenceTypes.hour`); running before hours leaves them degraded as `s1-unknown` until the next run's ref-heal pass. Non-cleared S1 AR rows are intentionally not migrated → `non_cleared_status` is expected in prod too: verify the count equals the frozen S1 non-cleared count, then allow. `perAccount[*].ok: true` for every account (count+sum match is built in, recomputed post-sweep). **Standard sync envelope (§10)** — identity is id_map `ledger-ar` (`s1 ledger_id ↔ chargePluginKey 'ar-<id>'`); the FIRST converted run mass-adopts every pre-sync s1-import row into id_map (content-equal → mapping only; drifted → update). Changed AR rows (status/amount/memo/account/ts) update in place; rows deleted-or-no-longer-Cleared in S1 sweep (hard-delete via charge-plugin key + mapping). Non-`ar-*` s1-import keys are foreign (t16/t19 allocations) and are never swept. | 58/58 rows, all 3 accounts ok, sums exact |
| 13 | `npx tsx scripts/s1-migration/load-log-notes.ts --migration-mode` | no expected reject allowance; allow only malformed timestamps or failed writes after triage | Workbook-approved `sirius_log` pairs become worker notes with complete S1 provenance and BAO tag sets. Six worker-applicable note types and Medium/Issue/Resolution tag types are provisioned idempotently with stable source identities; medium is optional, inbound/outbound controls the note type, and issue tags support multiple values. `material/material` becomes Document Detail; only the approved member eligibility email and appeal-denial letters use Email/Letter tags. Unresolved handlers are aggregate-only skips with no placeholder workers, and source edits, mapping repairs, out-of-scope moves, and source deletes reconcile through `id_map` entity `s1_log_note`. The loader is batch-oriented (page-level lookups, bulk chunked writes, set-based verification before fingerprints advance) and reports `detail.performance` (elapsed + rows/s + per-phase rates) and `detail.batching` for operator review. Progress heartbeats interleave `persist`/`verify` phases with the row scan. Completed `smf:notes`/`raw` ("Legacy Notes") rows are excluded at the staged-page query boundary on reruns and counted in `detail.immutableSkipped` — expect a rerun to show `created/updated/deleted = 0` with `unchanged + immutableSkipped` equal to the first run's total; a failed initial import keeps a NULL fingerprint and stays retryable. Deletion sweeping treats every staged immutable row as still-current (query exclusion is never mistaken for source deletion). Triage/fallback: `--force-reconcile` disables the immutable skip and re-reconciles everything (`immutableSkipped = 0` expected on such runs). |
...
| 15 | `npx tsx scripts/s1-migration/load-users.ts` | `--allow-rejects missing_mail,invalid_mail,duplicate_user_email` (synthetic traps) | **After contacts/workers** (T27, active accounts only; uid 0/1 never migrate). Run clean first; triage every fatal class. Reconciliation report (`reconciliation` in output) lists `no_resolvable_worker` / `ambiguous_worker_email` annotations for staff review — annotations don't block accounts. Reruns first deactivate accounts blocked/deleted in S1 since the last run (`deactivatedBlocked`/`deactivatedDeleted`) and revoke their migration-owned worker link + role. When such an inactive, canonically mapped account still owns an email now assigned to an active S1 uid, T27 preserves the old UUID/history/mapping, retires only its `users.email` to a deterministic non-deliverable value, and loads the current owner in the same run (`staleEmailsReleased` / `reconciliation.staleEmailReleases`). A live/current or noncanonical owner still rejects `email_owned_by_other_s1_user`; never allow it without triage. Role name collisions with pre-existing S2 roles bind to zero-permission `<name> (s1-migrated)` review roles (`roles.collisionDetails`) — review before cutover. `verifyFailures: 0`; `workerLinked` should cover the expected member-account share. | see §4.15 |
...
| `future_start_date` | relationships | not present | **RULED 2026-08-26 (mmcdermott4): allow.** Future-dated relationship starts are valid source data; the loader reports and skips them because S2 storage cannot persist a future start. |
| `wmb_create_failed` / `wmb_delete_failed` / `wmb_rel_repair_failed` | benefit-history | never | **NEVER allow:** these are unapplied S2 month writes and necessarily leave verify failures. Immediately run the read-only `scripts/oneoffs/s1-t17-wmb-create-failure-triage.ts` on the same target; future reports include a sanitized `code` per sample. Fix the missing dependency or loader defect, then rerun T17. |
...
| `handler_unresolved` / `handler_dangling` | cardchecks | ALLOWED (1 each, seeded staged fakes) | `handler_dangling` means the S1 target is absent from staging; `handler_unresolved` means it is staged but unmapped. Records with NO worker target are tolerated `noWorkerHandler` skips, not rejects. |
...
| log-notes | — | seeded approved and excluded `sirius_log` rows | ~1,000 staged rows/s insert-heavy at 20k-row bench scale (`scripts/oneoffs/s1-log-notes-bench.ts`, 512 MB heap cap; vs the ~22 rows/s per-row baseline observed at production scale); reruns exclude completed `smf:notes`/`raw` rows at the query boundary (`detail.immutableSkipped`) |
...
bootstrap-target'd database is already covered.
**Superseded 2026-08-26 (task 356): migration-mode T20 now bulk-writes
month-groups** instead of the per-row audited storage path that sustained
~45 staged payperiods/s on the wet production-profile rehearsal (projecting
~22 h). Production-shaped bench (`scripts/oneoffs/s1-t20-write-bench.ts` —
dev Neon, 100K staged payperiods → 50K month-groups, 512 MB heap cap, exact
SUM(hours)/row-count/sidecar parity + zero ledger delta asserted): scan
~20K staged rows/s, write ~1,850–2,150 month-groups/s (insert-heavy AND
pure-conflict rerun), verify ~3,200–4,000 keys/s → the full 3.6M-row T20
phase projects to **≈27 min**. The bench gates ≥450 groups/s (10× the
observed 45/s) and a ≤2.5 h projection — rerun it after any T20 write-path
change. Idempotent adopt-only re-runs are far faster than first loads. Use these, not the dev table, as the
...
- *t18 ledger* — identity is id_map entity `ledger-ar` (S1 `ledger_id` ↔
  ...
  s1-import keys belong to other loaders and are never swept. **Pay-period
  references (Task 414):** AR `ledger_reference` values targeting
  `sirius_payperiod` nodes resolve through t20's id_map `payperiod` crosswalk
  to `referenceType='hour'` → `worker_hours.id` — which is why **t20 runs
  before t18** in the fleet; the ref-heal pre-pass converges rows imported
  before the crosswalk existed on ordinary runs, and the one-time
  `repair-hour-links.ts` (below) backfills bulk pre-order data. Every run
  also re-checks ALREADY-LINKED hour rows against the current crosswalk
  (`detail.refHeal.hourDrift`): a payperiod retargeted in S1 repoints the
  link and a deleted payperiod degrades it back to `s1-unknown` — source
  fingerprints alone would fast-skip both forever.
...
- *t20 hours* — aggregates (payperiods → monthly rows) can't fingerprint
  ...
  In `--migration-mode`, flushes write through the bulk storage op
  `bulkUpsertWorkerHoursMigration`: 500-row multi-`INSERT … ON CONFLICT`
  batches on (worker, employer, year, month, day) updating ONLY
  migration-owned `hours` + `employment_status_id` — row ids and staff-owned
  `home`/`job_title` survive conflicts — and it refuses to run unless BOTH
  charge-plugin and notification suppression are active (fail-closed guard
  inside the storage layer, not the loader). Per-row audit snapshots and
  hour-saved side effects are replaced by one aggregate evidence line per
  batch plus per-flush bulk worker-scan invalidation and denorm stale
  marking; each flush stamps exactly the keys it persisted and verify reads
  back in batches, so an interrupted run resumes with the same command and
  stale cleanup stays unreachable until a fully verified pass.
- *t20 crosswalk* — after verify, contributing payperiod nids upsert into
  id_map entity `payperiod` (nid → monthly `worker_hours.id`, many nids per
  row; OVERWRITE semantics repoint moved periods); fully-verified runs retire
  mappings not restamped this run (same gate as stale hours cleanup).
- Dev smoke: `npx tsx scripts/s1-migration/dev/smoke-money-sync.ts --phase
  payments|ledger|hours|repair|parity` proves update/sweep convergence per
  loader (incl. crosswalk populate/repoint/retire, hour-reference resolution,
  and the repair command's dry/wet/rerun behavior) plus a 0-drift
  `verify-balance-parity` run.

**One-time hour-link repair (Task 414 — NOT part of any load or sync).**
Targets already lifted with the OLD ledger-before-hours order carry
s1-import AR rows degraded to `referenceType='s1-unknown'` with the payperiod
nid stashed in `data.s1ReferenceNid`; the BAO hourly plugin cannot see them
as the historical billing base, so a post-migration staff save of migrated
hours double-bills. Repair sequence (idempotent, safe to rerun):

```
npx tsx scripts/s1-migration/repair-hour-links.ts          # dry run (default): report only
# review the aggregate REPORT counts: linked / alreadyLinked / unresolved /
# ambiguous / mismatch / missingHours — unsafe matches are refused, never guessed
npx tsx scripts/s1-migration/repair-hour-links.ts --wet    # apply links + duplicate reconcile
npx tsx scripts/s1-migration/repair-hour-links.ts --wet    # rerun: linked=0, adjustmentsCreated=0
```

- *Link section:* relinks degraded rows whose stashed nid resolves in the
  `payperiod` crosswalk AND whose staged pay-period worker/employer/period
  agree with the target hours row; anything unresolved/ambiguous/mismatched is
  counted and skipped. Rows keep `chargePlugin='s1-import'` +
  `data.s1ReferenceNid` (identifiable S1 facts).
- *Duplicate section:* for linked imported history coexisting with a
  post-migration native BAO full base, the WET run re-executes the BAO hourly
  reconcile per (hours row, config) under notification suppression, producing
  at most ONE auditable correcting adjustment (`data.repairSource`); dry run
  reports affected pairs only (the plugin is never invoked dry). Skip with
  `--skip-duplicates` to link only. Aggregate-only output — no worker
  names/nids/amounts printed.
