/**
 * Baseline script — Sirius dev Repl — 2026-05-24.
 *
 * Re-runs the structural drift fix-ups for every enabled component table and
 * every core table. The previous baseline (`sirius-dev-20260518`, version
 * 1002) was supposed to install named foreign-key and unique constraints
 * across the dev database, but the live database is once again missing a
 * large set of unnamed FKs (cardchecks, dispatch, comm_*, employer_*,
 * events, options_*, trust_*, wizard_*, worker_*, ws_*) and four UNIQUE
 * constraints (trust_wmb, trust_wmb_scan_queue, worker_hours, worker_msh),
 * so the startup drift gate is refusing to boot.
 *
 ...
 *
 * Registered as a CORE migration at version 1003 (reserved baseline range,
...
async function up(): Promise<void> {
  ...
  logger.info("Baseline sirius-dev-20260524 complete", {
    service: "baseline",
...
const migration: Migration = {
  ...
  name: "baseline_sirius_dev_20260524",
  description:
    ...
    "enabled component table and every core table. Required because the dev DB " +
    "was once again missing the named constraints the startup drift gate expects.",
  up,
};
