...
const policy = definePolicy({
  id: 'esig.edit',
  description: 'Edit electronic signatures',
  scope: 'entity',
  entityType: 'esig',
  ...
  async evaluate(ctx: PolicyContext) {
    ...
    if (docType === 'cardcheck' && docId) {
      const hasAccess = await ctx.checkPolicy('cardcheck.edit', docId);
      if (hasAccess) {
        return { granted: true, reason: 'Has edit access to associated cardcheck' };
      }
    }
    
    return { granted: false, reason: 'No edit access to this esig' };
  },
});
