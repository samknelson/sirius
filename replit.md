...
-   **Worker Self-Registration**: A `/register` flow lets workers pre-verify with SSN + name + DOB and then complete account creation through the active auth provider (Clerk or Okta). For Okta tenants, after pre-verification Sirius creates the Okta user server-side via Okta's Users API (`POST /api/v1/users?activate=true`), which triggers Okta's standard activation email. The worker clicks the link, sets their password through Okta's hosted activation flow, then signs in normally; on first sign-in `checkUserAccess` links them to their worker record via single-contact-email match (worker role plus any `worker_user_roles_required` variable roles are assigned, `auth_identities` row tagged with `metadata.workerId`). Required env vars for Okta worker registration: `OKTA_API_TOKEN` (admin token with create-user + manage-group-membership permissions) and `OKTA_MEMBER_GROUP_ID` (or its alias `OKTA_NEW_USER_GROUP_ID`) — an Okta group already assigned to the OIDC app — newly-created workers are added to this group so they have app access on activation. Optional `OKTA_MEMBER_USER_TYPE` (or alias `OKTA_NEW_USER_TYPE`) assigns a non-default Okta user type at creation; accepts either the type id (e.g. `oty1abc...`) or the user type's name/displayName (e.g. `Member`), in which case Sirius resolves the id via `/api/v1/meta/types/user` and caches it. The ID token must include `sub` plus an `email` claim (and ideally `given_name`/`family_name`/`picture`). Each tenant runs ONE provider only (Clerk OR Okta). Gated by env var `AUTH_WORKER_SELF_REGISTRATION` (defaults to enabled); when disabled, `/api/auth/pre-verify-worker` and `/api/auth/complete-registration` return 403 and the login UI hides the Register link.
-   **Admin-driven Okta credentialing (employer & staff)**: Employer-contact users and staff/admin users have no self-serve registration. Admins click a single button — "Credential in Okta" — on the employer-contact user dialog (`POST /api/employer-contacts/:contactId/credential-okta`) or per-row on the user-management page (`POST /api/admin/users/:id/credential-okta`). The server looks up the email in Okta first via `GET /api/v1/users/{login}`. If a matching Okta user exists, Sirius writes the `auth_identities` row immediately (no activation email; user can sign in right away). If no Okta user exists, Sirius creates one in the persona's Okta group + user type and Okta dispatches its activation email. Re-clicking is a safe no-op ("already linked"). All flows require Okta to be the active provider and respect the storage-layer-only rule (writes go through `storage.authIdentities` and `storage.users`). Per-persona env vars: `OKTA_EMPLOYER_GROUP_ID` / `OKTA_EMPLOYER_USER_TYPE` and `OKTA_STAFF_GROUP_ID` / `OKTA_STAFF_USER_TYPE`. The member persona reads `OKTA_MEMBER_GROUP_ID` / `OKTA_MEMBER_USER_TYPE` and falls back to the legacy `OKTA_NEW_USER_GROUP_ID` / `OKTA_NEW_USER_TYPE` for backwards compatibility.
