...
-   **Dispatch System**: Manages dispatch jobs, types, listings, and detail pages. Features a plugin system to filter eligible workers based on configurable criteria using denormalized eligibility data. **Dispatch Job Groups** (`dispatch.job_group` component): Groups dispatch jobs with date ranges (`start_ymd`/`end_ymd`), optional `sirius_id` (unique, nullable text for external system linkage), optional JSON data, and full CRUD. Schema in `shared/schema/dispatch/job-group-schema.ts`, storage in `server/storage/dispatch/job-groups.ts`, API at `/api/dispatch-job-groups`. Storage includes `getBySiriusId(siriusId)` for lookup by external identifier. Frontend pages at `/dispatch/job_groups` (list), `/dispatch/job_group/new` (create), `/dispatch/job_group/:id` (detail with Details/Edit/Logs tabs). Layout uses `DispatchJobGroupLayout` + `useDispatchJobGroupLayout` hook. Staff-only access. **IMPORTANT**: The `data` (jsonb) column on `dispatch_job_group` is set programmatically by backend sync processes only. Never expose it for raw display or editing in the UI (Details, Edit, or New pages).
...
-   **Teamsters 631 Client Fetch**: Backend module at `server/modules/sitespecific/t631/client/fetch.ts` for communicating with the Teamsters 631 server. Core `t631Fetch(action, params?)` function handles authentication, request construction, and response parsing for all actions. Uses Basic Auth (account ID:access token) and JSON array body `[action, employerId, employerToken]`. Supported actions: `sirius_service_ping`, `sirius_edls_server_worker_list`, `sirius_dispatch_group_search`, `sirius_dispatch_facility_dropdown`, `sirius_edls_server_tos_list`. Single dispatch route at `POST /api/sitespecific/t631/client/fetch` accepts `{ action: "sirius_service_ping" }` (or any valid action from the list above). Gated by `admin` permission and both `edls` and `sitespecific.t631.client` components. Connection credentials stored as secrets (`SITESPECIFIC_T631_CLIENT_*`). Frontend test page at `/config/edls/t631-fetch` under Config > EDLS nav. **T631 Dispatch Job Group Sync**: Cron job `sitespecific-t631-dispatch-job-group-fetch` (handler at `server/cron/jobs/t631DispatchJobGroupFetch.ts`, sync logic via `syncJobGroups` at `server/modules/sitespecific/t631/client/sync-job-groups.ts`) fetches groups via `sirius_dispatch_group_search` and syncs them into `dispatch_job_group` table. Matches by `sirius_id` (populated from the remote record's UUID/nid); creates new groups, updates existing ones only when data has changed (skips unnecessary writes to avoid audit log noise). Tracks `unchanged` count alongside `created`/`updated`/`skipped`/`errors`. Stores raw T631 record in `data` jsonb column. Maps `title` → `name`, `field_sirius_datetime` → `startYmd`, `field_sirius_datetime_completed` → `endYmd`, `uuid`/`nid` → `siriusId`. Gated by `sitespecific.t631.client` component. Disabled by default in bootstrap. Supports test mode (dry run).
-   **Facilities** (`facility` component): Schema-managed table (`facilities`) for facility records, with FK reference to `contacts` (ON DELETE RESTRICT), `name`, optional unique `sirius_id` for external system linkage, and optional `data` jsonb for raw sync payloads. Schema in `shared/schema/facility/schema.ts`, storage in `server/storage/facility/facilities.ts` (includes `getBySiriusId`), routes at `/api/facilities` (`server/modules/facility/facilities.ts`). Staff for read/write, admin for delete. Component category `core`, disabled by default, `managesSchema: true`. The `data` jsonb is set programmatically by sync processes; not exposed for raw user editing in UI.
