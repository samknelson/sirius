...
-   **Ledger System**: Manages financial transactions including accounts, payments, and integrity reports. EA (Entity Account) access uses `ledger.ea.view` and `ledger.ea.edit` policies that delegate to entity-specific policies (`employer.ledger`, `worker.ledger`, `provider.ledger`) based on the EA's entity type. Staff-only admin pages (payment routes, payment types config, accounts list) use `staff` policy with `ledger` component requirement.
