...
-   **BTU Political Profile System**: Integrates with the Google Civic Information API to look up and store elected representatives for workers based on their addresses. Schema in `shared/schema/sitespecific/btu/political-schema.ts` with tables `sitespecific_btu_political_officials` and `sitespecific_btu_political_worker_reps`. Component ID: `sitespecific.btu.political`. API key via `GOOGLE_CIVICS_API_KEY` env secret. Features: Worker Political Profile tab for per-worker rep lookup, Political Profiles Report page (`/reports/political-profiles`) with search/filter/sort/CSV export, and a representative filter dropdown on the worker list that filters workers by their assigned elected official (uses EXISTS subquery against worker_reps table, guarded by component-enabled check). Storage layer follows the `tableExists()` / `COMPONENT_TABLE_NOT_FOUND` pattern. Nav entries use the `Landmark` icon.
