...
-   **Worker Management**: Comprehensive CRUD for workers, contacts, and benefits. The worker list uses server-side pagination (50 records per page) with debounced search and server-side filtering by employer, employer type, bargaining unit, benefit, and contact status. Cardcheck filters remain client-side.
...
-   **BTU Territories**: Manages geographical territory assignments for BTU-specific features. Schema includes `btu_territories` (id, sirius_id unique, name, data), `btu_territory_reps` (territory_id, contact_id with unique constraint), and `btu_territory_workers` (territory_id, worker_id unique). The BTU Employer Map includes a `territoryId` foreign key to associate employer map entries with territories. Territory selection is available in both individual record editing and bulk update operations. API routes at `/api/sitespecific/btu/territories` provide CRUD operations. Storage module at `server/storage/btu-territories.ts`.
