...
-   **Dashboard Plugin System**: Extensible widget framework. Server-side plugins live under `server/plugins/dashboard/plugins/*.ts` and declare the client component as `<plugin-id>:<ComponentName>`; the client auto-discovers components via `import.meta.glob` in `client/src/plugins/dashboard/registry.ts`. Pages consume `GET /api/plugins/dashboard/manifest` (unified plugin-manifest endpoint — see Task #208). **Every widget reads through a single front door:** `useDashboardContent` calls `GET /api/dashboard-plugins/:pluginId/content[/:action]` and treats 403/404 as `undefined`. Widgets must NOT receive `userPermissions`/`enabledComponents` or re-check gating client-side; `DashboardPluginProps` is intentionally narrowed to `{ userId, userRoles, componentProps }`. Server-side component (`plugin.componentId`) and access-policy (`plugin.requiredPolicy`) gating in `dashboardPluginRegistry.runContent` are the authoritative enforcement points. Cross-domain data belongs in the plugin's `content` resolver, not in ad-hoc routes. The generic `Card.tsx` widget accepts only `{ pluginId, action }` for data — the legacy `contentUrl` raw-fetch escape hatch has been removed (passing it is now a dev-mode `console.warn` and renders no body).
