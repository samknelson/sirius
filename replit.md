...
-   **Worker Self-Registration**: A `/register` flow lets workers pre-verify with SSN + name + DOB and then complete account creation through the active auth provider (Clerk or Okta). For Okta tenants, Okta-account creation itself is handled by Okta's hosted Self-Service Registration (SSR) — Sirius does not JIT-provision Okta users. Tenant Okta requirements: SSR enabled on the user-facing app (or the org), the OIDC client app assigned to all worker users, and the ID token must include `sub` plus an `email` claim (and ideally `given_name`/`family_name`/`picture`). On the Okta callback, the worker's Sirius user is found-or-created, the `worker` role plus any `worker_user_roles_required` variable roles are assigned, an `auth_identities` row is written tagged with `metadata.workerId`, and the contact email is overwritten by the IdP-supplied email (IdP wins). Each tenant runs ONE provider only (Clerk OR Okta). Gated by env var `AUTH_WORKER_SELF_REGISTRATION` (defaults to enabled); when disabled, `/api/auth/pre-verify-worker` and `/api/auth/complete-registration` return 403 and the login UI hides the Register link.
