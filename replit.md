...
-   **Bulk Campaigns**: Multi-channel campaign management system built on top of bulk messaging. `bulk_campaigns` table with status lifecycle (draft/queued/processing/completed/failed/aborted), audience type (worker/employer_contact), channel selection, and scheduling. Backend: full CRUD API at `/api/bulk-campaigns`, per-channel content upsert (`PUT /:id/messages/:medium`), audience import, readiness validation, queue/abort control, test-send, stats, and error detail endpoints. Frontend: "Bulk Message" buttons on Workers and Employer Contacts pages open a full-screen composer modal (`CampaignComposerModal`) targeting the current filtered/searched subset. The modal has tabbed workspaces: Email (WYSIWYG via SimpleHtmlEditor + subject), SMS (plain-text with 160-char segment counter/warning), Postal (template/PDF ref, color/double-sided options), Internal Log (mirror email toggle or custom summary). Token insertion at cursor (`[FirstName]`, `[LastName]`, etc.) across all text fields. Per-tab "Send Test" button for each channel. Scheduling toggle: Send Now or Schedule for Later with date/time picker and timezone selector. Final confirmation modal with "Proceed" button summarizes audience, channels, and schedule. Campaign Management Dashboard at `/campaigns` shows data grid with columns: Campaign Name, Creator, Scheduled Date, Channels, Audience Size, Progress %, Status. Clicking campaign name opens detail page; clicking "Failed" status links directly to errors tab. Detail page at `/campaigns/:id` with tabbed UI (Overview, Channels, Audience, Test, Errors) with URL-based tab selection (?tab=errors). Navigation: "Campaigns" link in header nav (gated by `bulk.edit` policy). Tokenization engine replaces tokens with contact data at delivery time. Campaign delivery managed by `bulkDeliver` cron job.
