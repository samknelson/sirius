...
-   **Bulk Campaigns**: Multi-channel campaign management system built on top of bulk messaging. `bulk_campaigns` table with status lifecycle (draft/queued/processing/completed/failed/aborted), audience type (worker/employer_contact), channel selection, and scheduling. Backend: full CRUD API at `/api/bulk-campaigns`, per-channel content upsert (`PUT /:id/messages/:medium`), audience import, readiness validation, queue/abort control, test-send, stats, and error detail endpoints. Frontend: Campaign dashboard at `/campaigns` (list with filters, progress bars), create page at `/campaigns/new` (name, audience type, channel selection), detail page at `/campaigns/:id` with tabbed UI (Overview, Channels, Audience, Test, Errors). Channels tab provides per-medium content editors with token insertion support. "New Campaign" buttons on Workers and Employer Contacts pages link to campaign creation with pre-selected audience type. Tokenization engine replaces `[FirstName]`, `[LastName]`, etc. with contact data at delivery time. Campaign delivery managed by `bulkDeliver` cron job. Navigation: "Campaigns" link in header nav (gated by `bulk.edit` policy).
