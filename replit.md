...
-   **SFTP Client Destinations**: Manages SFTP client destination configuration via `sftp_client_destinations` table. Gated by `system.sftp.client` component. Full CRUD API at `/api/sftp/client-destinations`. Admin-only with component enforcement on all routes. Frontend: list page at `/config/sftp/clients` (in ConfigurationLayout), detail/edit pages at `/config/sftp/clients/:id` and `/config/sftp/clients/:id/edit` (with `SftpClientLayout`). Schema defined in `shared/schema/system/sftp-client-schema.ts`, table created via `db:push:component`.
