...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get(
    "/api/workers/employers/summary",
    requireAuth,
    requirePermission("workers.view"),
    async (req, res) => {
      try {
        const workerEmployers =
          await storage.workers.getWorkersEmployersSummary();
  ...
  app.get(
    "/api/workers/benefits/current",
    requireAuth,
    requirePermission("workers.view"),
    async (req, res) => {
      try {
        const workerBenefits =
          await storage.workers.getWorkersCurrentBenefits();
        res.json(workerBenefits);
      } catch (error) {
        console.error("Failed to fetch worker current benefits:", error);
        res
          .status(500)
          .json({ message: "Failed to fetch worker current benefits" });
      }
    },
  );
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        ...
        if (email !== undefined) {
          const worker = await storage.workers.updateWorkerContactEmail(
            id,
            email,
          );
        ...
        // Handle birth date updates
          ...
          const worker = await storage.workers.updateWorkerContactBirthDate(
            id,
            birthDate,
          );
        ...
        // Handle gender updates
          ...
          const worker = await storage.workers.updateWorkerContactGender(
            id,
            gender,
            genderNota,
          );
        ...
        // Support both old format (name) and new format (nameComponents)
          ...
          const worker =
            await storage.workers.updateWorkerContactNameComponents(
              id,
              nameComponents,
            );
          ...
          const worker = await storage.workers.updateWorkerContactName(
            id,
            name.trim(),
          );
  ...
  app.get(
    "/api/employers",
    requireAuth,
    requireAccess(policies.employersView),
    async (req, res) => {
      try {
  ...
  app.get(
    "/api/employers/:id",
    requireAuth,
    requireAccess(policies.employerUser),
    async (req, res) => {
      try {
        const { id } = req.params;
  ...
  app.get(
    "/api/employers/:employerId/workers",
    requireAuth,
    requireAccess(policies.employerUser),
    async (req, res) => {
      try {
      ...
      try {
        ...
        if (name !== undefined) {
          if (!name || typeof name !== "string" || !name.trim()) {
            return res
              .status(400)
              .json({ message: "Employer name cannot be empty" });
          }
    ...
    async (req, res) => {
      try {
        ...
        if (!local || typeof local.enabled !== "boolean") {
          return res
            .status(400)
            .json({ message: "Invalid local configuration." });
        }

        if (!google || typeof google.enabled !== "boolean") {
          return res
            .status(400)
            .json({ message: "Invalid google configuration." });
        }
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        ...
        if (local) {
          const localCurrentSettings =
            await serviceRegistry.getProviderSettings("sms", "local");
          const existingLocalValidation =
            (localCurrentSettings as any)?.phoneValidation || {};
          const localValidationSettings = {
            ...existingLocalValidation,
            defaultCountry:
              local.defaultCountry ??
              existingLocalValidation.defaultCountry ??
              "US",
            strictValidation:
              local.strictValidation ??
              existingLocalValidation.strictValidation ??
              true,
          };
        ...
        const twilioCurrentSettings = await serviceRegistry.getProviderSettings(
          "sms",
          "twilio",
        );
        const existingTwilioValidation =
          (twilioCurrentSettings as any)?.phoneValidation || {};
        const twilioValidationSettings = {
          ...existingTwilioValidation,
          lookupType: twilio?.lookupType ??
            existingTwilioValidation.lookupType ?? [
              "line_type_intelligence",
              "caller_name",
            ],
          useLocalOnTwilioFailure:
