...
export function registerWorkerUserSettingsRoutes(
  ...
  app.get("/api/worker-user-settings", requireAccess(policies.admin), async (req, res) => {
    try {
  ...
  app.put("/api/worker-user-settings", requireAccess(policies.admin), async (req, res) => {
    try {
