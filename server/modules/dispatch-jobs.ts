...
export function registerDispatchJobsRoutes(
    ...
    async (req, res) => {
      try {
        ...
        if (
          ...
          dispatchJobStatusEnum.includes(status as any)
        ) {
        ...
        const result = await storage.dispatchJobs.getPaginated(
          page,
          limit,
          filters,
        );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const employer = await storage.employers.getEmployer(
        ...
        if (status !== undefined) {
          if (!dispatchJobStatusEnum.includes(status)) {
            res
              .status(400)
              .json({
                message: `Invalid status. Must be one of: ${dispatchJobStatusEnum.join(", ")}`,
              });
            return;
          }
      ...
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Failed to update job running status";
        ...
        res
          .status(500)
          .json({ message: "Failed to update job running status" });
      }
    },
  );
  ...
  app.get(
    "/api/dispatch-jobs/:id/eligible-workers",
    dispatchComponent,
    requireAccess("admin"),
    async (req, res) => {
      try {
        ...
        const {
          limit: limitParam,
          offset: offsetParam,
          siriusId: siriusIdParam,
          name: nameParam,
          excludeWithDispatches: excludeParam,
        } = req.query;
        ...
        const result = await eligibleWorkersStorage.getEligibleWorkersForJob(
          id,
          limit,
          offset,
  ...
  app.get(
    "/api/dispatch-jobs/:id/eligible-workers-sql",
    dispatchComponent,
    requireAccess("admin"),
    async (req, res) => {
      try {
        ...
        const eligibleWorkersStorage = createDispatchEligibleWorkersStorage();
        const result = await eligibleWorkersStorage.getEligibleWorkersForJobSql(
          id,
          limit,
          offset,
          Object.keys(filters).length > 0 ? filters : undefined,
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch eligible workers SQL" });
      }
    },
  );

  app.get(
    "/api/dispatch-jobs/:id/check-eligibility/:workerId",
    dispatchComponent,
    requireAccess("admin"),
    async (req, res) => {
      try {
        ...
        const eligibleWorkersStorage = createDispatchEligibleWorkersStorage();
        const result = await eligibleWorkersStorage.checkWorkerEligibility(
          id,
          workerId,
        );
