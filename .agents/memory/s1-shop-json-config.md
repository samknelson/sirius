...
**Special employers (ruled 2026-08-09):** DISABILITY (12639701), DISABILITY-INDUSTRIAL (12639702), COBRA Employer (13226164) are ordinary shops in S1 — COBRA/Disability coverage is NOT an election type (that vocab is coverage tiers). COBRA elections carry policy "COBRA" (13226124); DISABILITY elections have no policy by design (falls to policy default in S2). Election node type is `sirius_trust_worker_election`, not `sirius_trust_election`.
