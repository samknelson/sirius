...
- A surface differs only in WHICH declared fields it authors. It supplies
  templates for the keys it writes; an unsupplied key is not rendered, not
  previewed and not required. That is how a bulk postal editor offers only a
  description while the medium still declares a letter body.
