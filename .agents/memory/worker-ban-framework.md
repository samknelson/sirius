...
- A boot-time legacy-ban conversion is optional cleanup only when the legacy literal remains directly enforceable. Serialize seed+conversion in one transaction, validate the canonical option's behavior, and prove every target row reached it before commit.
  **Why:** production can retain historical constraints or triggers that reject or silently alter a soft-reference rewrite; taking the app offline or weakening active bans is worse than leaving an enforceable literal for reviewed repair.
  **How to apply:** preflight the live type/constraints/triggers/data, fail soft with exact PostgreSQL diagnostics for incompatible cleanup, and reserve fatal boot refusal for a state where neither legacy nor canonical bans can be enforced safely.
