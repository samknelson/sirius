...
- Derived codes collapse punctuation/case, so DISTINCT real-S1 term names can derive the SAME code ("Nonbinary" vs "Non-binary" → NONBINARY) — name adoption misses and create hits the unique constraint. Loader must also adopt by derived code (both tids map to the shared row) AND register mid-run-created rows in its lookup maps so within-run duplicates adopt too.
