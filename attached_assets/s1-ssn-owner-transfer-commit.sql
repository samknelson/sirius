-- S1 rehearsal target: guarded SSN ownership transfer COMMIT
--
-- Destructive: transfers the existing SSN from the stale worker to the
-- canonical worker. Does not print the SSN, modify Sirius IDs, delete workers,
-- reparent history, alter mappings, or change reject allowlists.
--
-- Run only after s1-ssn-owner-transfer-rollback.sql returns READY_TO_COMMIT
-- against the same confirmed target and no intervening contacts/workers run
-- changes either worker.
