...
function WorkerPaymentContent() {
  ...
  const beginPayment = useMutation({
    ...
    onSuccess: (data: Intent) => {
      ...
      if (data.status === "succeeded" || data.status === "processing" || data.status === "failed") {
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {amount <= 0.005 ? <p className="text-sm text-muted-foreground" data-testid="text-worker-payment-paid">You have no balance due.</p> :
          ...
          {(methods.data ?? []).length === 0 ? <p className="text-sm text-muted-foreground" data-testid="text-worker-no-payment-methods">Add a payment method to continue.</p> :
