...
function TrustProviderLogsContent() {
  ...
  const { data: logs = [], isLoading } = useQuery<WinstonLog[]>({
    queryKey: ["/api/trust/provider", provider?.id, "logs", moduleFilter, operationFilter, startDate, endDate],
    queryFn: async () => {
      if (!provider?.id) {
        throw new Error("Provider ID is required");
      }
      const queryString = params.toString();
      const url = `/api/trust/provider/${provider.id}/logs${queryString ? `?${queryString}` : ''}`;
      const response = await fetch(url);
      if (!response.ok) {
        throw new Error("Failed to fetch trust provider logs");
      }
        ...
        <CardContent>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              No log entries found for this trust provider.
            </div>
          ) : (
