...
import { FileText, Loader2, Maximize2, RefreshCw, Save, Trash2, TriangleAlert } from "lucide-react";
...
export default function LetterTemplateDetailPage() {
  ...
  const [form, setForm] = useState({ name: "", siriusId: "", medium: "email" as MediumName, contextIds: [] as string[], content: {} as Record<string, string> });
  ...
  useEffect(() => {
    if (template.data && !initialized) {
      setForm({ name: template.data.name, siriusId: template.data.siriusId ?? "", medium: template.data.medium, contextIds: template.data.contextIds, content: template.data.content ?? {} });
  ...
  const save = useMutation({
    mutationFn: () => apiRequest("PATCH", `/api/admin/letter-templates/${id}`, { name: form.name.trim(), siriusId: form.siriusId.trim() || null, medium: form.medium, contextIds: form.contextIds, content: form.content }),
    onSuccess: (next: LetterTemplate) => { queryClient.setQueryData([`/api/admin/letter-templates/${id}`], next); queryClient.invalidateQueries({ queryKey: ["/api/admin/letter-templates"] }); setDirty(false); },
  });
  const remove = useMutation({
    mutationFn: () => apiRequest("DELETE", `/api/admin/letter-templates/${id}`),
    onSuccess: () => {
      queryClient.removeQueries({ queryKey: [`/api/admin/letter-templates/${id}`] });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/letter-templates"] });
      setLocation("/admin/letter-templates");
    },
  });
  const errorMessage = remove.error
    ? getApiErrorMessage(remove.error, "Could not delete this template.")
    : save.error
      ? getApiErrorMessage(save.error, "Could not save this template.")
      : template.error
        ? getApiErrorMessage(template.error, "Could not load this template.")
        : null;
  ...
  return <div className="space-y-6">
    ...
    <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Delete this letter template?</AlertDialogTitle>
          <AlertDialogDescription>
            “{template.data.name}” will no longer be available in Template Studio. This cannot be undone.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
