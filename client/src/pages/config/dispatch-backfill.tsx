...
export default function DispatchBackfillPage() {
  usePageTitle("Dispatch Eligibility Backfill");
    ...
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-semibold" data-testid="text-page-title">
          Dispatch Eligibility Backfill
        </h1>
        <p className="text-muted-foreground">
          Rebuild dispatch eligibility data for a plugin. Use this after changing
          configuration or importing data to recompute who is eligible.
        </p>
      </div>
      ...
      {sortedPlugins.length === 0 ? (
        <Card>
          <CardContent className="py-8 text-center text-muted-foreground" data-testid="text-no-plugins">
            No dispatch eligibility plugins are available to backfill.
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-3">
          {sortedPlugins.map(plugin => {
            ...
            return (
              <Card key={plugin.id} data-testid={`card-backfill-${plugin.id}`}>
