...
interface WorkerUserSettingsResponse {
  required: string[];
  optional: string[];
}
...
export default function WorkerUserSettingsPage() {
  ...
  const { data, isLoading: settingsLoading } = useQuery<WorkerUserSettingsResponse>({
    queryKey: ["/api/worker-user-settings"],
  });
  ...
  const updateSettingsMutation = useMutation({
    mutationFn: async (settings: { required: string[]; optional: string[] }) => {
      return apiRequest("PUT", "/api/worker-user-settings", settings);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/worker-user-settings"] });
      toast({
        title: "Settings Updated",
        description: "Worker user role settings have been updated successfully.",
      });
    },
    onError: (error: any) => {
      toast({
        title: "Update Failed",
        description: getApiErrorMessage(error, "Failed to update worker user settings."),
        variant: "destructive",
      });
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Configure which roles are required or optional for worker users
          </p>
        </div>
        <Button
          onClick={handleSave}
          disabled={!hasChanges || updateSettingsMutation.isPending}
          data-testid="button-save-worker-settings"
        >
      ...
      <Alert>
        ...
        <AlertDescription>
          Required roles will be automatically assigned to worker users.
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              These roles will be automatically assigned to all worker users
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              These roles can be manually assigned to worker users as needed
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
