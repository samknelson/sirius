...
function CronJobRunContent() {
  ...
  const completedOutput = completedRun ? parseOutputData(completedRun.output) : null;
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Latest manual execution completed at{" "}
                  {completedRun.completedAt
                    ? format(new Date(completedRun.completedAt), "MMM d, yyyy HH:mm:ss")
                    : "—"}
                </CardDescription>
              </div>
              <Badge variant={completedRun.mode === "test" ? "outline" : "default"} data-testid="badge-run-mode">
                {completedRun.mode === "test" ? "Test" : "Live"}
              </Badge>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            {completedRun.status === "error" && completedRun.error && (
              <div className="rounded-md bg-destructive/10 p-3" data-testid="text-run-error">
                <p className="text-sm font-medium text-destructive mb-1">Error</p>
                <p className="text-sm text-destructive whitespace-pre-wrap">{completedRun.error}</p>
              </div>
            )}

            {completedOutput && (
              <div className="space-y-3" data-testid="container-run-output">
                <div className="text-sm text-muted-foreground">
                  Execution time: {completedOutput.executionTimeSec}s
                </div>
                {completedOutput.summary && Object.keys(completedOutput.summary).length > 0 && (
                  <div className="rounded-md border p-3 space-y-2">
                    <p className="text-sm font-medium">Summary</p>
                    {Object.entries(completedOutput.summary).map(([key, value]) => (
                      <div key={key} className="text-sm" data-testid={`text-summary-${key}`}>
                        <span className="font-medium">{key}:</span> {formatSummaryValue(value)}
                      </div>
                    ))}
                  </div>
                )}
...
export default function CronJobRun() {
  return (
    <CronJobLayout activeTab="run">
      <CronJobRunContent />
    </CronJobLayout>
  );
}
