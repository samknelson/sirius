...
function WorkerUserContent() {
  ...
  // Fetch user data
  const { data, isLoading, error } = useQuery<WorkerUserResponse>({
    queryKey: ["/api/workers", worker.id, "user"],
  });
  ...
  useEffect(() => {
    ...
    } else if (data && !data.hasUser && contact) {
      // Pre-fill from contact name if creating new user
      setFirstName(contact.given || "");
      setLastName(contact.family || "");
      setIsActive(true);
  ...
  const saveUserMutation = useMutation({
    mutationFn: async () => {
      return apiRequest("POST", `/api/workers/${worker.id}/user`, {
        firstName,
        lastName,
        isActive,
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/workers", worker.id, "user"] });
      toast({
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-muted-foreground text-center max-w-md">
            This worker must have an email address before a user account can be created.
            Please add an email address to this worker first.
          </p>
        </CardContent>
      </Card>
    );
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {data.hasUser && (
            <Alert>
              ...
              <AlertDescription>
                A user account exists for this worker with email <strong>{data.contactEmail}</strong>
              </AlertDescription>
            </Alert>
          )}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  data-testid="input-worker-user-first-name"
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  data-testid="input-worker-user-last-name"
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                disabled
                className="bg-muted"
                data-testid="input-worker-user-email"
              />
              <p className="text-xs text-muted-foreground">
                Email cannot be changed here. Update the worker's email to change the user's email.
              </p>
            </div>
            ...
            <div className="flex items-center space-x-2">
              <Switch
                ...
                data-testid="switch-worker-user-active"
              />
          ...
          {requiredRoles.length > 0 && (
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                These roles are required for all worker users and cannot be removed.
              </p>
          ...
          {optionalRoles.length === 0 && requiredRoles.length > 0 && (
            <Alert>
              <AlertDescription>
                No optional roles are configured. All worker users will have only the required roles.
              </AlertDescription>
            </Alert>
          )}
          ...
          {optionalRoles.length === 0 && requiredRoles.length === 0 && (
            <Alert>
              <AlertDescription>
                No required or optional roles are configured for worker users. 
                Configure roles in the Worker User Settings configuration page.
              </AlertDescription>
            </Alert>
          )}
          ...
          <div className="flex justify-end gap-2">
            <Button
              ...
              data-testid="button-save-worker-user"
            >
...
export default function WorkerUserPage() {
  return (
    <WorkerLayout activeTab="user">
      <WorkerUserContent />
    </WorkerLayout>
  );
}
