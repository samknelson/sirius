...
function EmployerContactUserContent() {
  ...
  useEffect(() => {
    if (data?.hasUser && data.user) {
      ...
      setFirstName(employerContact.contact.given || "");
      setLastName(employerContact.contact.family || "");
      setIsActive(true);
  ...
  const saveUserMutation = useMutation({
    mutationFn: async () => {
      return apiRequest("POST", `/api/employer-contacts/${employerContact.id}/user`, {
        firstName,
        lastName,
        isActive,
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/employer-contacts", employerContact.id, "user"] });
      toast({
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {requiredRoles.length > 0 && (
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                These roles are required for all employer users and cannot be removed.
              </p>
          ...
          {optionalRoles.length === 0 && requiredRoles.length > 0 && (
            <Alert>
              <AlertDescription>
                No optional roles are configured. All employer users will have only the required roles.
              </AlertDescription>
            </Alert>
          )}
...
export default function EmployerContactUserPage() {
  return (
    <EmployerContactLayout activeTab="user">
      <EmployerContactUserContent />
