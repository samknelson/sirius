...
function EmployerLogsContent() {
  ...
  const { data: logs = [], isLoading } = useQuery<WinstonLog[]>({
    queryKey: ["/api/employers", employer.id, "logs", moduleFilter, operationFilter, startDate, endDate],
    queryFn: async () => {
      const queryString = params.toString();
      const url = `/api/employers/${employer.id}/logs${queryString ? `?${queryString}` : ''}`;
      const response = await fetch(url);
      if (!response.ok) {
        throw new Error("Failed to fetch employer logs");
      }
        ...
        <CardContent>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              No log entries found for this employer.
            </div>
          ) : (
...
export default function EmployerLogs() {
  return (
