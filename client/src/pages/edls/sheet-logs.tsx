...
function EdlsSheetLogsContent() {
  ...
  const { data: logs = [], isLoading } = useQuery<WinstonLog[]>({
    queryKey: ["/api/edls/sheets", sheet.id, "logs", moduleFilter, operationFilter, startDate, endDate],
    queryFn: async () => {
      const queryString = params.toString();
      const url = `/api/edls/sheets/${sheet.id}/logs${queryString ? `?${queryString}` : ''}`;
      const response = await fetch(url);
      if (!response.ok) {
        throw new Error("Failed to fetch sheet logs");
      }
        ...
        <CardContent>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              No log entries found for this sheet.
            </div>
          ) : (
