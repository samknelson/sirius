...
export interface CommDetailContentProps {
  /** Communication record to render. */
  commId: string;
  /**
   * Optional action rendered beside the status badge. Status-changing
   * actions belong to the surface that owns them (the detail page), never to
   * this read-only view.
   */
  statusAction?: ReactNode;
  /**
   * Rendered instead of the built-in loading / not-found states. The detail
   * page keeps its own framing for those, so it passes its own fallback.
   */
  renderFallback?: (state: { isLoading: boolean; notFound: boolean }) => ReactNode;
}

/**
 * Read-only rendering of one communication record: the message summary
 * (medium, status, recipient, bodies, data) plus its activity log with
 * per-entry details. Shared by the communication detail page and by the
 * dialog opened from other surfaces, so the two cannot drift apart.
 *
 * Its own queries mean it fetches when — and only when — it is mounted.
 */
export function CommDetailContent({ commId, statusAction, renderFallback }: CommDetailContentProps) {
  ...
  if (commLoading || commError || !comm) {
    const state = { isLoading: commLoading, notFound: !commLoading };
    if (renderFallback) return <>{renderFallback(state)}</>;
    return commLoading ? (
      <div className="text-center py-8 text-muted-foreground" data-testid="text-comm-detail-loading">
        Loading communication details...
      </div>
    ) : (
      <div className="text-center py-8" data-testid="text-comm-detail-error">
