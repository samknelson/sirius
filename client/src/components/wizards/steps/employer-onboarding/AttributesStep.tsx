...
import { Settings, Building, Building2, Factory, Store, Warehouse, Home, Landmark, Hospital } from "lucide-react";
...
export function AttributesStep({ wizardId, data }: AttributesStepProps) {
  const { toast } = useToast();
  const [typeId, setTypeId] = useState<string | null>(data?.typeId || null);
  const [industryId, setIndustryId] = useState<string | null>(data?.industryId || null);
  const [selectedBenefitIds, setSelectedBenefitIds] = useState<string[]>(data?.benefitIds || []);
  const [selectedLedgerAccountIds, setSelectedLedgerAccountIds] = useState<string[]>(data?.ledgerAccountIds || []);
  ...
  const saveAttributes = (overrides?: any) => {
    updateMutation.mutate({
      typeId: overrides?.typeId !== undefined ? overrides.typeId : typeId,
      industryId: overrides?.industryId !== undefined ? overrides.industryId : industryId,
      benefitIds: overrides?.benefitIds !== undefined ? overrides.benefitIds : selectedBenefitIds,
      ledgerAccountIds: overrides?.ledgerAccountIds !== undefined ? overrides.ledgerAccountIds : selectedLedgerAccountIds,
    });
  };
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <CardDescription>Configure the employer type, industry, and benefit fund participation</CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {trustBenefits.length > 0 && (
          <div>
            <Label className="text-sm font-medium mb-3 block">Trust Benefit Funds</Label>
              ...
              {ledgerAccounts.map((account: any) => (
                <div key={account.id} className="flex items-center space-x-3">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      const newIds = checked
                        ...
                        : selectedLedgerAccountIds.filter((id: string) => id !== account.id);
                  ...
                  <Label htmlFor={`ledger-${account.id}`} className="cursor-pointer">
                    {account.name}
                    {account.description && (
                      <span className="text-muted-foreground ml-2 text-xs">- {account.description}</span>
                    )}
                  </Label>
                </div>
              ))}
