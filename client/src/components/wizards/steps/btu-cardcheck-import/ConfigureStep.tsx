  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Card check import settings have been saved.",
      });
    },
    onError: (error: Error) => {
      toast({
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select which card check type to create from this import file
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              {loadingDefinitions ? (
                <div className="text-sm text-muted-foreground">Loading card check types...</div>
              ) : (
                <Select
              ...
              <p className="text-sm text-muted-foreground">
                All imported card checks will be created as signed records of this type.
              </p>
            </div>
          </div>
          ...
          <Alert>
            ...
            <AlertTitle>How the import works</AlertTitle>
            <AlertDescription>
              Each row in your file will be matched to a worker by their BPS Employee ID.
              If a matching worker is found, a signed card check record will be created with the
              signature date from the file. Workers that cannot be found will be reported in the results.
              If a bargaining unit column is mapped, it will be used; otherwise the worker's existing
              bargaining unit will apply.
            </AlertDescription>
          </Alert>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-2 text-green-600 dark:text-green-400">
              ...
              <span className="text-lg">Configuration saved</span>
            </div>
            <div className="mt-2 text-sm text-muted-foreground">
              <p>Card Check Type: <strong>{selectedDefinition.name}</strong></p>
            </div>
          </CardContent>
        </Card>
      )}
