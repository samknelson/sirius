...
/**
 * `preview` step for the BTU signature import wizard. The match is
 * computed server-side by the plugin's `submit` handler (through the fixed
 * dispatcher submit route) and persisted to `wizard.data.previewData`,
 * which arrives here on the reactive `data` prop. No wizard-specific route.
 */
export function SigPreview({ wizardId, step, data }: WizardStepComponentProps) {
  const { toast } = useToast();
  const previewData: PreviewData | null = data?.previewData || null;
            ...
            <div className="space-y-4">
              ...
              {previewData.matched.some((m) => m.existingCardcheckHasEsig) && (
                <Alert>
                  ...
                  <AlertDescription>
                    Files matching workers who already have an e-signature on
                    their card check will still create a new offline e-sig
                    record, but the existing card check will not be updated.
                  </AlertDescription>
                </Alert>
              )}
