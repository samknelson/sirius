...
/**
 * `results` step for the BTU scraper import wizard. Reads the persisted
 * `processResults` off the reactive `data` prop and offers a client-side
 * CSV export. No wizard-specific route.
 */
export function ScrapeResults({ data }: WizardStepComponentProps) {
  ...
  const exportResults = () => {
    ...
    for (const r of processResults.processedRows) {
      lines.push(
        `"${r.cardcheckId}","${r.externalId}","${r.workerId}","${r.action}","${r.esigId || ""}"`,
      );
    }
    for (const e of processResults.errors) {
      lines.push(
        `"${e.cardcheckId}","${e.externalId}","","error: ${e.error}",""`,
      );
    }
    downloadCsv(
      lines.join("\n"),
  ...
  const linked = processResults.processedRows.filter(
    (r) => r.action === "linked",
  );
  const skipped = processResults.processedRows.filter(
    (r) => r.action === "skipped",
  );
        ...
        <CardHeader>
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle data-testid="text-results-title">
                  Scraper Import Complete
                </CardTitle>
                <CardDescription>
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="p-4 border rounded-lg text-center">
              <div className="text-3xl font-bold" data-testid="text-final-total">
            ...
            <div className="p-4 border rounded-lg text-center">
              <div
                ...
                data-testid="text-final-skipped"
              >
                {skipped.length}
              </div>
          ...
          <Accordion type="multiple" className="space-y-2">
            {linked.length > 0 && (
              <AccordionItem
                value="linked"
                className="border rounded-lg border-green-200 dark:border-green-900"
              >
                <AccordionTrigger
                  className="px-4 hover:no-underline"
                  data-testid="accordion-linked"
                >
            ...
            {processResults.errors.length > 0 && (
              <AccordionItem
                value="errors"
                className="border rounded-lg border-red-200 dark:border-red-900"
              >
                <AccordionTrigger
                  className="px-4 hover:no-underline"
                  data-testid="accordion-errors"
                >
                  <div className="flex items-center gap-2">
