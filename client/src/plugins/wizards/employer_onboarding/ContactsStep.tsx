...
/**
 * Employer contacts step: manage the contact list and flag which contacts
 * get promoted to login users. Persists through the dispatcher submit
 * route (no wizard-specific endpoint).
 */
export function ContactsStep({ wizardId, step, data }: WizardStepComponentProps) {
  ...
  const updateMutation = useMutation({
    mutationFn: async (updatedContacts: Contact[]) =>
      apiRequest("POST", `/api/wizards/${wizardId}/dispatch/${step.id}/submit`, {
        input: { contacts: updatedContacts },
      }),
    onSuccess: () => {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>
                Add contacts for this employer and optionally create user
                accounts for them
              </CardDescription>
            </div>
          </div>
          ...
          <div className="flex justify-between items-center pt-4 border-t">
            <p className="text-sm text-muted-foreground">
              ...
              {contacts.filter((c) => c.promoteToUser).length > 0 && (
                <span>
                  {" "}
                  ({contacts.filter((c) => c.promoteToUser).length} will get user
                  accounts)
                </span>
              )}
            </p>
