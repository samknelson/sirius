...
export function BuildingRepResults({ data }: WizardStepComponentProps) {
  ...
  const exportResults = () => {
    ...
    for (const a of processResults.createdAssignments) {
      lines.push(
        `"${a.name}","${a.badgeId}","${a.workerId}","${a.employerName}","${a.bargainingUnitName}","Created",""`,
      );
    }
    for (const a of processResults.alreadyAssignedRows || []) {
      lines.push(
        `"${a.name}","${a.badgeId}","${a.workerId}","${a.employerName}","${a.bargainingUnitName}","Skipped","Already assigned"`,
      );
    }
    for (const s of processResults.skippedDuringProcess || []) {
      lines.push(
        `"${s.name}","${s.badgeId}","${s.workerId}","${s.employerName}","","Skipped","${s.reason}"`,
      );
    }
  ...
  const totalNotCreated =
    alreadyAssignedRows.length +
    skippedDuringProcess.length +
  ...
  for (const e of processResults.errors) {
    ...
    if (!errorsByReason.has(reason)) errorsByReason.set(reason, []);
    errorsByReason.get(reason)!.push({ name: e.name, badgeId: e.badgeId });
  }
  for (const [reason, rows] of Array.from(errorsByReason.entries()).sort(
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold" data-testid="text-result-reasons">
              {allReasons.length}
            </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      {item.hasDetailColumns &&
                        item.rows.some((r) => r.bargainingUnitName) && (
                          <TableHead>Bargaining Unit</TableHead>
                        )}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {item.rows.map((r, rIdx) => (
                      <TableRow
                        key={rIdx}
                        data-testid={`row-reason-${idx}-${rIdx}`}
                      >
                        <TableCell>
                          {r.workerId ? (
                            <Link
                        ...
                        {item.hasDetailColumns && (
                          <TableCell>{r.employerName || "-"}</TableCell>
                        )}
                        {item.hasDetailColumns &&
                          item.rows.some((row) => row.bargainingUnitName) && (
                            <TableCell>{r.bargainingUnitName || "-"}</TableCell>
                          )}
                      </TableRow>
                    ))}
                  </TableBody>
