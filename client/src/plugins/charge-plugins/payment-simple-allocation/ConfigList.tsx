...
export default function PaymentSimpleAllocationConfigList({ pluginId }: ChargePluginConfigProps) {
  ...
  const getAccountNames = (accountIds?: string[]) => {
    if (!accountIds || accountIds.length === 0) return "No accounts selected";
    return accountIds
      .map(id => {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Default configuration applied to all payments on selected accounts
          </CardDescription>
        </CardHeader>
        <CardContent>
          {globalConfig ? (
              ...
              <div className="flex items-center justify-between p-4 border rounded-md">
                <div className="space-y-1">
                  ...
                  <div className="mt-2 space-y-1 text-sm text-muted-foreground">
                    <p><strong>Accounts:</strong> {getAccountNames(globalConfig.settings.accountIds)}</p>
                    <p className="text-xs mt-2">
                      When a payment on one of these accounts is saved with status "cleared", 
                      a corresponding ledger entry will be created.
                    </p>
                  </div>
                </div>
